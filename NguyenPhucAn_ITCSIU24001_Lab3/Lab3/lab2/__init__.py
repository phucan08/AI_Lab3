...
from tkinter import Tk, Frame, Button, BOTH, OptionMenu, StringVar, Text, END, PhotoImage, Toplevel, Label
...
class Lab2:
    ...
    def __init__(self):
        ...
        self.result_window_payload = None
    ...
    def get_agent_result_data(self):
        if self.agent is None:
            return None

        if hasattr(self.agent, "result_data") and self.agent.result_data:
            return self.agent.result_data

        if getattr(self.agent, "alive", True):
            return None

        algorithm = getattr(self.agent, "search_algorithm", type(self.agent).__name__)
        if hasattr(self.agent, "get_result_grid"):
            grid = self.agent.get_result_grid()
        else:
            grid = "{}x{}".format(self.vacuum_env.env_x, self.vacuum_env.env_y)

        return {
            "algorithm": algorithm,
            "grid": grid,
            "steps": getattr(self.agent, "steps", "N/A"),
            "nodes_explored": getattr(self.agent, "nodes_explored", "N/A"),
            "score": getattr(self.agent, "score", getattr(self.agent, "performance", "N/A")),
        }

    def show_result_form(self, result_data):
        if result_data is None:
            return

        if self.result_window is not None and self.result_window.winfo_exists():
            if self.result_window_payload == result_data:
                return
            self.result_window.destroy()

        self.result_window_payload = dict(result_data)
        self.result_window = Toplevel(self.root)
        self.result_window.title("Simulation Result")
        self.result_window.resizable(width=False, height=False)

        content = Frame(self.result_window, padx=16, pady=16)
        content.pack(fill=BOTH, expand=True)

        Label(content, text="Algorithm: {}".format(result_data["algorithm"]), anchor="w", justify="left").pack(fill="x")
        Label(content, text="Grid: {}".format(result_data["grid"]), anchor="w", justify="left").pack(fill="x")
        Label(content, text="Steps: {}".format(result_data["steps"]), anchor="w", justify="left").pack(fill="x")
        Label(content, text="Nodes explored: {}".format(result_data["nodes_explored"]), anchor="w", justify="left").pack(fill="x")
        Label(content, text="Score: {}".format(result_data["score"]), anchor="w", justify="left").pack(fill="x", pady=(0, 8))

        Button(content, text="Close", command=self.result_window.destroy).pack(pady=(10, 0))

    def check_for_result(self):
        result_data = self.get_agent_result_data()
        if result_data is None:
            return

        self.stop()
        self.show_result_form(result_data)
    ...
    def create_sim(self):
        ...
        if selected_agent == AGENT_MYVACUUMAGENT:
            agent = MyVacuumAgent(*self.grid_dims_getter(), self.append_log, self.algorithm_getter())
